...
public:
  ...
  Grid(i_tile tileSize, LatLonBounds extent, unsigned short int rootTiles = 1):
    mTileSize(tileSize),
    mExtent(extent),
  ...
  crsToTile(const LatLon &latLon, i_zoom zoom) const {
  ...
  tileBounds(const TileCoordinate &coord) const {
    ...
    const LatLon lowerLeft = pixelsToCrs(pxLowerLeft, coord.zoom),
      upperRight = pixelsToCrs(pxUpperRight, coord.zoom);

    return LatLonBounds(lowerLeft, upperRight);
  }
